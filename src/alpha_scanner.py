"""
Alpha Scanner — V6 Core Component (FIXED)

Scans all active Polymarket markets, gets AI probability estimates,
and flags opportunities with 20%+ edge.

FIX: Added Bayesian anchoring + sanity checks to prevent 72% false positive rate.
"""
...
REASONING: [1-2 sentences, especially if you disagree with market]"""
        ...
        try:
            ...
            return {'estimate': market_price, 'confidence': 'LOW', 'reasoning': 'Timeout', 'filtered': True}
        except Exception as e:
            logger.debug(f"Error getting AI estimate: {e}")
            return {'estimate': market_price, 'confidence': 'LOW', 'reasoning': str(e), 'filtered': True}
    ...
    def _parse_ai_response(self, response: str, market_price: float) -> dict:
        ...
        # Extract estimate
        estimate_match = re.search(r'ESTIMATE:\s*(\d+(?:\.\d+)?)', response, re.IGNORECASE)
        if estimate_match:
    ...
    def _apply_sanity_checks(self, result: dict, market_price: float) -> dict:
        ...
        # CHECK 1: Cap maximum edge at configured limit (default 50%)
        ...
        # CHECK 2: Extreme markets (<5% or >95%) need higher edge and confidence
        extreme_threshold = self.cfg.alpha_extreme_threshold
        extreme_min_edge = self.cfg.alpha_extreme_min_edge
        
        is_extreme = market_price < extreme_threshold or market_price > (1 - extreme_threshold)
        if is_extreme:
            recalc_edge = abs(result['estimate'] - market_price)
            if recalc_edge < extreme_min_edge:
                result['estimate'] = market_price
                result['filtered'] = True
                result['filter_reason'] = f'Extreme market ({market_price:.1%}), edge {recalc_edge:.1%} < {extreme_min_edge:.1%}'
            elif result['confidence'] == 'LOW':
                result['estimate'] = market_price
                result['filtered'] = True
                result['filter_reason'] = 'Low confidence on extreme market'
        
        # CHECK 3: Require reasoning for significant disagreement (>10% edge)
    ...
    async def scan(self, max_markets: Optional[int] = None) -> ScanResult:
        ...
        total = len(markets)
        logger.info(f"Scanning {total} markets for alpha (threshold: {self.cfg.alpha_threshold:.0%})...")
        
        for i, market in enumerate(markets):
            if (i + 1) % self.cfg.log_interval == 0:
                logger.info(f"Progress: {i + 1}/{total} markets scanned, {len(opportunities)} opportunities found, {filtered_count} filtered")
            ...
            # Check if filtered by sanity checks
        ...
        logger.info(f"Scan complete in {duration:.1f}s — {len(opportunities)} opportunities ({alpha_rate:.1f}% alpha rate), {filtered_count} filtered")
...
if __name__ == "__main__":
    ...
    for opp in result.opportunities[:20]:
        print(f"\n{opp.direction.value} @ {opp.market_price:.1%} → AI: {opp.ai_estimate:.1%} ({opp.edge_pct:.1f}% edge)")
